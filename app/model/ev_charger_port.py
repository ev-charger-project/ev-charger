...
class EVChargerPort(BaseModel, table=True):
    power_plug_type_id: uuid.UUID = Field(
        foreign_key="powerplugtype.id", primary_key=True, index=True, nullable=False
    )
    power_output_id: uuid.UUID = Field(
        foreign_key="poweroutput.id", primary_key=True, index=True, nullable=False
    )
    ev_charger_id: uuid.UUID = Field(
        foreign_key="evcharger.id", primary_key=True, index=True, nullable=False
    )
